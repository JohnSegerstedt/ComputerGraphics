...
void drawScene(GLuint currentShaderProgram, const mat4 &viewMatrix, const mat4 &projectionMatrix, const mat4 &lightViewMatrix, const mat4 &lightProjectionMatrix)
{
	...
	// Task 2
	mat4 scalingMatrix = scale(vec3(0.5f));
	mat4 translationMatrix = translate(vec3(0.5f));

	mat4 lightMatrix = lightProjectionMatrix * lightViewMatrix * inverse(viewMatrix);
	mat4 translatedLightMatrix = translationMatrix * scalingMatrix * lightMatrix;
	labhelper::setUniformSlow(currentShaderProgram, "lightMatrix", translatedLightMatrix);
...
void gui()
{
	...
	ImGui::SliderFloat("Factor", &polygonOffset_factor, 0.0f, 100.0f);
